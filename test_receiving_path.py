...
try:
    payload = json.load(sys.stdin)  # espera JSON por stdin
except json.JSONDecodeError as e:
    print(json.dumps({"error": "invalid_json", "message": str(e)}))
    sys.exit(1)

# validar formato (esperamos una lista)
if not isinstance(payload, list):
    print(json.dumps({"error": "expected_list", "received_type": type(payload).__name__}))
    sys.exit(2)
...
#Se regresa un json que diga que todo chido
salida = {
    "success": True,
    "result": "Recorrido terminado!"
}
# escribir JSON de vuelta a stdout
